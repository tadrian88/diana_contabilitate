...
docker compose exec -T postgres psql -U diana -d postgres -c "DROP DATABASE IF EXISTS diana_spv_connection_e2e WITH (FORCE)" >/dev/null
docker compose exec -T postgres psql -U diana -d postgres -c "CREATE DATABASE diana_spv_connection_e2e OWNER diana" >/dev/null

cd backend
export DATABASE_URL="postgresql://diana:diana@127.0.0.1:5442/diana_spv_connection_e2e?sslmode=disable"
