...
export function RuleDetailPage() {
  ...
  if (isError) return <div className="card p-10 text-center"><h2 className="font-bold">Regula nu a putut fi încărcată</h2><p className="mt-2 text-sm text-[var(--text-secondary)]">Repository-ul demonstrativ a returnat o eroare.</p></div>
  ...
  return <div className="space-y-5">
    ...
    <section className="flex items-start justify-between gap-8"><div><div className="flex flex-wrap items-center gap-2"><Badge tone={rule.scope === 'GLOBAL' ? 'neutral' : 'info'}>{RULE_SCOPE_LABELS[rule.scope]}</Badge><Badge tone="neutral">{RULE_CATEGORY_LABELS[rule.category]}</Badge><Badge tone={current ? 'success' : 'neutral'}>Versiunea {selected.version} · {current ? 'Curentă' : 'Istorică'}</Badge></div><h2 className="mt-3 text-2xl font-bold tracking-tight">{rule.name}</h2><p className="mt-1 text-sm text-[var(--text-secondary)]">{rule.reference}{client ? ` · ${client.name}` : ' · aplicabilitate globală de bază'}</p></div>{current && <div className="flex gap-3"><Button variant="secondary" onClick={() => setVersionOpen(true)}><GitBranch className="size-4" />Creează versiune nouă</Button>{rule.scope === 'GLOBAL' && <Button onClick={() => setOverrideOpen(true)}><UserRound className="size-4" />Creează override pentru client</Button>}</div>}</section>
    ...
    <section className="card p-5"><div className="flex items-center gap-2 text-[var(--warning)]"><Scale className="size-5" /><h3 className="font-bold">Context legal demonstrativ</h3></div><p className="mt-3 text-sm text-[var(--text-secondary)]">{selected.legalBasis}</p><p className="mt-2 text-xs text-[var(--text-muted)]">Regula folosește date UX fictive și nu reprezintă o concluzie contabilă sau fiscală.</p></section>
